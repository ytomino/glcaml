...
let load_gl_textures () =
	let s = load_bmp "data/glass.bmp" in
...
let callback k =
	...
	match (k.sym) with
       ...
       | K_B -> if !blend = false then (glEnable gl_blend; glDisable gl_depth_test) else (glDisable gl_blend; glEnable gl_depth_test); blend := not !blend;
       | _ -> ()
